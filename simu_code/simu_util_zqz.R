gendata_simu_lowrank <-function (seed = 1, n = 300, p = 50, d=20, q = 6, rank0=3, rho = 1, sigma2_eps=0.1){
  ...
  X <- matrix(rpois(n * p, lambda = mu), n, p)
    ...
    mu_no <- exp( H0 %*% t(B0) + matrix(mu0, n, p, byrow = T) + epsi)
...
PLNPCA_runno <- function(X_count, covariates, q,  Offset=rep(1, nrow(X_count)), workers=NULL){
  require(PLNmodels)
  if(!is.null(workers)){
    future::plan("multisession", workers = workers)
  }
...
mrrr_run <- function(Y, X, rank0, q, family=list(poisson()),
  ...
  control = list(epsilon = epsilon, sv.tol = sv.tol, maxit = maxIter,
                 ...
                 conv.obj = TRUE)
  fit.mrrr <- mrrr(Y=Y, X=X, family = family, familygroup = familygroup,
