...
double calELBO(const mat& X_count, const vec& a, const mat& Z, const mat& Mu_y, const mat& S_y,
               const vec& invLambda, const mat& B, const mat& bbeta, const mat& Mu_h,
  ...
  mat dX = (Mu_y - Z*bbeta.t() - Mu_h * B.t()) % repmat(sqrt(invLambda.t()), n, 1);
  ...
  ELBO = pois_term1 + pois_term2 + dimreduction_term1 + dimreduction_term2 + entropy;
  return ELBO;
}
...
// joint method
mat update_bbeta_joint(const mat& Z, const mat&Y, const mat& S_bar, const mat& B, const mat& S_y, const int& rank_use, const bool& fast_svd=true){ // can be speeded up !!!!
  
  // Perform singular value decomposition of X
  int n = Y.n_rows, d = Z.n_cols;
  mat C_ls = pinv(Z.t() * Z) * Z.t()* Y; // d*p
  vec bsb1 = decomp(S_bar, B);
  mat dX = Y-Z*C_ls;
  vec lambdaVec =  trans(mean(dX % dX + S_y)) + bsb1/n; // tSigma_eps
  dX.reset();
  bsb1.reset();
...
void VB_Estep(const mat& X_count, const vec& a, const mat& Z, mat& Mu_y,  mat& S_y,
              const vec& invLambda, const mat& B, const mat& bbeta, mat& Mu_h,
              mat& S_h, const mat& Sigma_h, const int& fast_version){
  ...
  // double elbo0 = calELBO(X_count, a, Z, Mu_y, S_y, invLambda, B, bbeta, Mu_h, S_h, Sigma_h);
  if(fast_version==1){ // fast_version==1 means Laplace approximation plus Taylor approximation.
    // Method 2: Laplace approximation + Tylor approximation
    // double to_ij =0.0, tz_ij;
    // for(i = 0; i<n ; ++i){
    //   for(j = 0; j<p; ++j){
    //     to_ij = a(i) * exp(Mu_y(i,j));
    //     tz_ij = tau(j)+ as_scalar(Z.row(i) * bbeta.row(j).t()+ Mu_h.row(i) * B.row(j).t());
    ...
    mat tmp_mat = Z * bbeta.t() + Mu_h * B.t();
  ...
  Mu_h = (Mu_y - Z*bbeta.t()) * (B % repmat(invLambda, 1, q)) * S_h;
...
Rcpp::List Reduced_Rank_COAP(const arma::mat& X_count, const arma::vec& a, const arma::mat& Z, const int& rank_use,
  ...
  for(iter = 1; iter < maxIter; ++iter){
    ...
    // VB E-step
    VB_Estep(X_count, a, Z, Mu_y, S_y, invLambda, B, bbeta, Mu_h, S_h, Sigma_h, fast_version);
    ...
    //double elbo1 = calELBO(X_count, a, Z, Mu_y, S_y, invLambda, B, bbeta,  Mu_h, S_h, Sigma_h);
    //update B
    B = trans(Mu_y - Z * bbeta.t()) * Mu_h * inv_sympd(Mu_h.t() * Mu_h + S_bar);
    // double elbo2 = calELBO(X_count, a, Z, Mu_y, S_y, invLambda, B, bbeta,  Mu_h, S_h, Sigma_h);
    // Rprintf("dB= %4f \n", elbo2 - elbo1);
    ...
    if(sep_opt_beta){
      ...
      bbeta = update_bbeta_joint( Z, tY, S_bar, B, S_y, rank_use, fast_svd);
    }
    //bbeta = trans(Mu_y - Mu_h * B.t()) * Z * inv_sympd(Z.t()*Z);
    
    // double elbo3 = calELBO(X_count, a, Z, Mu_y, S_y, invLambda, B, bbeta,  Mu_h, S_h, Sigma_h);
    // Rprintf("dbbeta= %4f \n", elbo3 - elbo2);
  
    
    // update Sigma_h
    Sigma_h = (Mu_h.t() * Mu_h + S_bar)/n;
    // double elbo5 = calELBO(X_count, a, Z, Mu_y, S_y, invLambda, B, bbeta,  Mu_h, S_h, Sigma_h);
    // Rprintf("dSigma= %4f\n", elbo5 - elbo3);
    
    // update Lambda
    dX = Mu_y -  Z * bbeta.t()- Mu_h * B.t();
    // vec bsb = diagvec(B * S_bar * B.t()); // this can be speeded up by using decomp() function.
    bsb = decomp(S_bar, B);
    Lambda =  trans(mean(dX % dX + S_y)) + bsb/n;
    invLambda = 1.0 / Lambda;
    // double elbo6 = calELBO(X_count, a, Z, Mu_y, S_y, invLambda, B, bbeta,  Mu_h, S_h, Sigma_h);
    // Rprintf("dLambda= %4f\n", elbo6 - elbo5);
    
    
    ELBO_vec(iter) = calELBO(X_count, a, Z, Mu_y, S_y, invLambda, B, bbeta, Mu_h, S_h, Sigma_h);
  ...
  mat alpha = pinv(Z.t() * Z) * Z.t()* Mu_h; // d*q
  mat H_new = Mu_h - Z * alpha;
